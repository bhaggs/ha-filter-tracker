...
automation:
  - alias: "Daily filter check from calendar"
    trigger:
      - platform: calendar
        entity_id: calendar.filter_tracker
        event: start
        offset: "-24:0:0"  # 1 day before
    action:
      - service: notify.mobile_app
        data:
          title: "Filter Replacement Tomorrow"
...
### Filter entities not appearing

1. Check that the integration is loaded: Settings → System → Logs
2. Restart Home Assistant
3. Check that the integration shows in Settings → Devices & Services
